...
class VISION():
    ...
    def pan(self, diff=None, pos=None):
        if diff is not None:  # Adds a difference
            self.panpos += diff
        elif pos is not None:  # Changes the position
            self.panpos = pos

    # Updates the head position
    def headmotion(self):
        self.headtilt += (
            np.sign(self.tiltpos - self.headtilt) *
            np.min([abs(self.tiltpos - self.headtilt) * self.clk / 3.,
                    self.headspd]) / self.clk
        )

        self.headpan += (
            np.sign(self.panpos - self.headpan) *
            np.min([abs(self.panpos - self.headpan) * self.clk / 3.,
                    self.headspd]) / self.clk
        )

        self.headtilt = np.min([self.maxtilt,
                                np.max([self.mintilt, self.headtilt])
                                ])

        self.headpan = np.min([self.maxpan,
                               np.max([self.minpan, self.headpan])
                               ])
    ...
    def draw(self, where):
        ...
        for d in [dn, df]:
            for a in [self.hfov, -self.hfov]:
                x = (self.robot.x + d *
                     np.cos(np.radians(-self.robot.rotate + self.headpan + a))
                     )
                y = (self.robot.y + d *
                     np.sin(np.radians(-self.robot.rotate + self.headpan + a))
                     )
                pts.append((x, y))
        ...
        for a in [0, 3]:
            for b in [1, 2]:
                pygame.draw.line(where, (255, 255, 255), pts[a], pts[b], 1)
        ...
        #     angle = -self.robot.rotate + self.headpan + point[1]
        ...
        #     pygame.draw.circle(where, self.robot.color,
        # (int(x), int(y)), 2, 0)
    ...
    #         # Comp ute the point distance
    #         dist = point[0] + np.random.normal(0,point[0]/10.)
    #         # Compute the direction of the point
    #         angle = -self.robot.rotate + self.headpan
    # + point[1] + np.random.normal(0,2)
    ...
    #         elif f[0][0] <= x and x <= f[0][1] and f[1][0]
    # <= y and y <= f[1][1]:
    #             ret.append(1)
    #         else:
    #             ret.append(0)
    ...
    #         if (1 + sp.erf((min(df,700)-dist)/(np.sqrt(2)*10))) *
    # (1 - sp.erf((max(10,dn)-dist)/(np.sqrt(2)*1))) / 4 >= np.random.random():
    ...
    # def GetBall(self):
    #     dist = hypot(self.robot.x-self.robot.ball.x,
    # self.robot.y-self.robot.ball.y)
    #     ang = -degrees(atan2(self.robot.ball.y-self.robot.y,
    # self.robot.ball.x-self.robot.x))-self.robot.rotate
    ...
    def VisionProcess(self):
        # When the time is up, and the jump flag was activated,
        # the robot jumps to the initial position.
        # if time.time() % 60 < 1:
        #     if self.jump:
        ...
        if bhv >= 300 and bhv < 400:
            ...
            i = bhv - 300
            np.save('../../Experiments/robot' +
                    str(self.exp * 100 + i), np.array(self.save))
